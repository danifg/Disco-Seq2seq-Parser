...
class DepParsingStateMachine():
    ...
    def get_valid_actions(self):
        ...
        if len(self.stack) > 1 and self.num_words>1 and self.stack[-1] not in self.reduced_nodes and self.stack[-1] not in self.nts\
        and self.stack[-2] not in self.reduced_nodes and self.stack[-2] not in self.nts and self.swaps_left[int(self.stack[-2])-1]>0:
            #print(self.swaps_left)
