...
void TopDownOracle::load_oracle(const string& file, bool is_training) {
  cerr << "Loading in-order oracle from " << file << " [" << (is_training ? "training" : "non-training") << "] ...\n";
  ...
  const int kTERM_INT = ad->convert("TERM");	
   ...
   const string kSHIFT_PUNTO = "SHIFT..";
   const string kSHIFT_COMA = "SHIFT,,";		
   const int kSHIFT_PUNTO_INT = ad->convert("SHIFT..");
   const int kSHIFT_COMA_INT = ad->convert("SHIFT,,");
  ...
  while(getline(in, line)) {
    ...
    while(getline(in, line)) {
      ...
      if (line.find("RE(") == 0) {
        ...
        termc++;
} else if (line == kSHIFT_PUNTO) {
        cur_acts.push_back(kSHIFT_PUNTO_INT);
        termc++;
} else if (line == kSHIFT_COMA) {
        cur_acts.push_back(kSHIFT_COMA_INT);
        termc++;
      } else if (line == kTERM){
