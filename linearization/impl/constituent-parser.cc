...
/*if(params.debug){
	...
	if(params.fast){
		 if(stack_buffer_split>0){
			s_att.push_back(tanh(affine_transform({s_attbias, s_input2att, input[stack_buffer_split-1], s_h2att, prev_h})));
		  	s_input.push_back(input[stack_buffer_split-1]);
		}else if(stack_buffer_split==0){
...
if(params.debug){
	...
	for(unsigned i = 0; i < s_see.size(); i ++){
        ...
        num_trans++;
	if(pos==s_see.size()-1)num_st0++;
        if(s_see.size()>1 && pos==s_see.size()-2)num_st1++;
	if(s_see.size()>2 && pos==s_see.size()-3)num_st2++;
	if(s_see.size()>3 && pos<=s_see.size()-4)num_stm++;

	att_st0=att_st0+s_see[s_see.size()-1];
	st0++;
	if(s_see.size()>1){
		 att_st1=att_st1+s_see[s_see.size()-2];
		st1++;	
	}
	if(s_see.size()>2){
		 att_st2=att_st2+s_see[s_see.size()-3];
		st2++;	
	}
	if(s_see.size()>3){
		for(unsigned i = 0; i < s_see.size()-3; i ++){
		 	att_stm=att_stm+s_see[i];
			stm++;	
		}
	}
}
///////////////////////////////////////////////////////////////////////////////////////////
      ...
      if(params.fast){
			if(stack_buffer_split<sent.size()){
				b_att.push_back(tanh(affine_transform({b_attbias, b_input2att, input[stack_buffer_split], b_h2att, prev_h})));
				b_input.push_back(input[stack_buffer_split]);
			 }else if(stack_buffer_split==sent.size())
			{
...
if(params.debug){
	...
	if(pos==0)num_bt0++;
	if(b_see.size()>1 && pos==1)num_bt1++;
	if(b_see.size()>2 && pos==2)num_bt2++;
	if(b_see.size()>3 && pos>=3)num_btm++;

	att_bt0=att_bt0+b_see[0];
	bt0++;
	if(b_see.size()>1){
		 att_bt1=att_bt1+b_see[1];
		bt1++;	
	}
	if(b_see.size()>2){
		 att_bt2=att_bt2+b_see[2];
		bt2++;	
	}
	if(b_see.size()>3){
		for(unsigned i = 3; i < b_see.size(); i ++){
		 	att_btm=att_btm+b_see[i];
			btm++;	
		}
	}
}
...
/*if(params.debug) {
    ...
    if (train && action_count != correct_actions.size()) {
      cerr << "Unexecuted actions remain but final state reached!"<< action_count<<" "<<correct_actions.size()<<"\n";
      abort();
    }
...
//============================================================================================================
  ...
  if (params.train) {
	...
	if((tot_seen / corpus.size())>11.0)break;
	...
	cerr<<system(cmd_1)<<"\n";
	  ...
	  ostringstream part_os;
          part_os << "parser"
                ...
                << '_' << params.attention_hidden_dim
                << "-pid" << getpid()
                << "-part" << (tot_seen/corpus.size()) << ".params_"<<newfmeasure;
	...
	if(params.samples !=0){
		...
		if(params.debug){
			cerr << " " << num_trans << " trans"<< endl;
			cerr << " " << (double)(num_st0*100)/num_trans << " topstack0"<< endl;
			cerr << " " << (double)(num_st1*100)/num_trans << " topstack1"<< endl;
			cerr << " " << (double)(num_st2*100)/num_trans << " topstack2"<< endl;
			cerr << " " << (double)(num_stm*100)/num_trans << " <topstack"<< endl;
			cerr << " " << (double)(num_bt0*100)/num_trans << " topbuffer0"<< endl;
			cerr << " " << (double)(num_bt1*100)/num_trans << " topbuffer1"<< endl;
			cerr << " " << (double)(num_bt2*100)/num_trans << " topbuffer2"<< endl;
			cerr << " " << (double)(num_btm*100)/num_trans << " <topbuffer"<< endl;
			cerr << " ______________________________"<<endl;
			cerr << " " << (double)(att_st0)/st0 << " value att topstack0"<< endl;
			cerr << " " << (double)(att_st1)/st1 << " value att topstack1"<< endl;
			cerr << " " << (double)(att_st2)/st2 << " value att topstack2"<< endl;
			cerr << " " << (double)(att_stm)/stm << " value att <topstack"<< endl;
			cerr << " " << (double)(att_bt0)/bt0 << " value att topbuffer0"<< endl;
			cerr << " " << (double)(att_bt1)/bt1 << " value att topbuffer1"<< endl;
			cerr << " " << (double)(att_bt2)/bt2 << " value att topbuffer2"<< endl;
			cerr << " " << (double)(att_btm)/btm << " value att <topbuffer"<< endl;
		}
