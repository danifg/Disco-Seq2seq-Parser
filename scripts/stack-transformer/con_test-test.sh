...
for single_task  in $(python -c "print(' '.join('$TASK_TAG'.split('+')))");do
	...
	discodop eval $ORACLE_FOLDER/test.discbracket ${results_path}.discbracket $ORACLE_FOLDER/proper.prm --fmt=discbracket --disconly | tail -n 3 | head -n 1 | awk '{print "LAS: " $4}' > ${results_path}.disco
	cat ${results_path}.disco
