...
for test_model in $(find $checkpoints_folder -maxdepth 1 -iname 'checkpoint[0-9]*.pt' | sort -r);do
    ...
    [ ! -f "${std_name}.actions" ] && [ -f "$test_model" ] && \
        bash scripts/stack-transformer/con_test.sh \
            $config \
            $test_model \
            $std_name
...
python scripts/stack-transformer/con_rank_model.py \
    --link-best \
    --checkpoints $checkpoints_folder --score-names las --no-split-name
